...
4. Подготовлены артефакты для автоматической обработки:
   - список понятий в `concepts.json`;
    - скрипт простановки перекрестных ссылок в markdown;
    - шаблонный скрипт генерации страниц через LLM API;
    - скрипт сборки страницы словаря терминов.
   - таблицы узлов и связей онтологии.
5. Сгенерированы markdown-страницы в каталоге `WEB/6_1_reasonable_spending/articles`.
...
### Шаблон генерации через LLM API
Файл: `src/generate_pages_template.py`

Назначение:
- создает markdown-страницы по понятиям из `concepts.json`;
...
3. SPARQL-запросы требуют ручной валидации сущностей `wd:Q...`, потому что разные базы знаний иногда расходятся в деталях.
